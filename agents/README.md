...
Stage- and date-aware agents **resolve the org's real CRM stages from its own data** (`dealtrace crm-stages`) instead of hardcoding labels like "Closed Won" — see [../docs/crm-stages.md](../docs/crm-stages.md). Each agent's exact trigger is in its page under **## Trigger**.
