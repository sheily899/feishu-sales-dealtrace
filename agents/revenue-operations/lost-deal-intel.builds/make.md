# Lost-Deal Intel — Make build

Guided build for a Make (Integromat) scenario, mirroring the canonical spec
([`../lost-deal-intel.md`](../lost-deal-intel.md)). A verified, importable Make **blueprint JSON**
...
2. **Your CRM** (Salesforce "Search Records" / HubSpot "Search Deals", or **HTTP → Make a request**)
   - Query Opportunities: `IsClosed = true AND IsWon = false AND CloseDate within last 7 days`.
   - Output: name, account, owner, amount, stage-at-loss, loss reason.
3. **Iterator** over the returned deals (so each deal's calls are fetched), then
   **HTTP → Make a request** to your call recorder's API for that deal's/account's calls. (Recorder
   that only exports transcripts? Pull from where you store them, or pre-ingest via the dealtrace adapters.)
...
5. **HTTP → Make a request** to `https://api.anthropic.com/v1/messages`
   - Headers: `x-api-key`, `anthropic-version: 2023-06-01`. Body: `model: claude-sonnet-4-5`,
     `max_tokens: 3000`, a single user message = the **Lost-Deal operating prompt**
     (see [`../lost-deal-intel.md`](../lost-deal-intel.md)) with the aggregated deals + calls mapped in.
