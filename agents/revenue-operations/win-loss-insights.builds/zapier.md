# Win Loss Insights — Zapier build

Zapier has no import file, so this is the exact Zap to assemble. It mirrors the canonical spec
([`../win-loss-insights.md`](../win-loss-insights.md)). Swap the example apps for the ones you use.

## Steps

1. **Trigger — Schedule by Zapier** → *Every Week*, Monday, 8:00 AM.
2. **Action — your CRM (e.g. Salesforce "Find Record(s)" / HubSpot "Find Deals")**
   - Find Opportunities where `IsClosed = true` and `CloseDate` within the last 30 days.
   - Return: name, account, owner, amount, stage, close date, won/lost flag, cycle length.
   - (For trend, add a second find for days 31-60.)
3. **Action — your call recorder (or Webhooks by Zapier → GET)**
   - Fetch the calls tied to those closed deals/accounts. If your recorder only exports transcripts,
...
4. **Action — Anthropic (or "AI by Zapier", or Webhooks → POST to `https://api.anthropic.com/v1/messages`)**
   ...
   - Prompt = the **Win Loss Insights operating prompt** (see [`../win-loss-insights.md`](../win-loss-insights.md) → How it works / Output), with the closed deals from step 2 and the calls from step 3 mapped in.
