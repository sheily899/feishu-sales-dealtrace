# Objection Drilldown — Make build

Guided build for a Make (Integromat) scenario, mirroring the canonical spec
([`../objection-drilldown.md`](../objection-drilldown.md)). A verified, importable Make **blueprint
...
5. **HTTP → Make a request** to `https://api.anthropic.com/v1/messages`
   - Headers: `x-api-key`, `anthropic-version: 2023-06-01`. Body: `model: claude-sonnet-4-5`,
     `max_tokens: 3000`, a single user message = the **Objection Drilldown operating prompt**
     (see [`../objection-drilldown.md`](../objection-drilldown.md)) with this week's + last week's calls mapped in.
