...
DealTrace 从飞书群聊中提取客户需求、顾虑、待办和风险，并跨轮保存事项状态与原文证据。它用于记录客户当前关注的问题，以及这些问题在后续对话中的变化。

## Demo

> **[打开离线 Demo](https://sheily899.github.io/feishu-sales-dealtrace/)**：浏览器直接打开，无需安装、命令或 API Key。

![DealTrace 工作台](docs/assets/demo-workbench.png)

![工作台总览](docs/assets/demo-workbench-overview.png)

公开 Demo 使用内置脱敏的 10 条连续销售对话和预置分析报告，不调用模型，也不消耗 API 额度。

### 脱敏后的真实飞书对话

下图来自本项目实际接入的飞书群聊，并已进行隐私脱敏：参与者姓名统一替换为“客户”和“销售”，头像替换为匿名头像。截图用于说明工作台接收的真实消息形态，不属于公开评测 Golden 数据集，也不会暴露个人身份信息。

![脱敏后的真实飞书对话](docs/assets/feishu-chat-anonymized.png)

体验路径：打开公开链接，查看对话、事项、状态和原文依据。
...
| 模块 | 输入 | 输出 | 存在原因 | 失败影响 |
|---|---|---|---|---|
| 飞书接入 | 飞书群消息事件 | 标准消息 | 隔离外部系统格式 | 无法接收或错误接收消息 |
| 角色识别 | 发送者 ID、角色配置 | 客户/销售角色 | 同一句话由不同角色说含义不同 | 客户需求与销售承诺混淆 |
| 对话分析 | 完整聊天历史 | 分类、评分、建议 | 提供销售视角的全局分析 | 报告不准确，但不应直接修改状态 |
| 事项提取 | 旧状态、新消息 | IssueOperation | 语言表达难以靠规则穷举 | 漏掉事项或提出错误变化 |
| 状态校验 | 模型操作、历史事项、原文 | 接受或拒绝结果 | 不直接相信模型 | 错误状态进入客户档案 |
| SQLite 存储 | 消息、报告、状态变化 | 状态版本历史 | 支持跨轮追踪和恢复 | 重启后历史丢失 |
| 工作台 | 消息、报告、状态、证据 | 可视化页面 | 查看结论和原文 | 系统只能停留在后台结果 |
| 评测模块 | Golden 事项、实际输出 | P/R/F1、证据率 | 判断改动是否真的有效 | 容易凭感觉改 Prompt |

模型只提出分析结果，状态模块负责校验和保存最终状态；下一轮消息读取上一版状态。

## 能做什么

- 规范化飞书群聊事件，区分客户和销售角色；
- 识别需求、顾虑、风险、相关角色、承诺、待办和下一步；
- 跨天保留未解决事项，支持新增、更新、接受替代方案和解决；
...
排查：左侧没有群时检查群 ID 是否在白名单；没有分析结果时检查模型 Key 和网络；只想体验页面时使用 8765 端口的“离线演示”。
...
这些数字来自小规模内部评测，用于验证短周期多轮追踪能力，不代表数月或数百条消息场景下的生产效果。
