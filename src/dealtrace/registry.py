...
from .models import CallType, Framework, Outcome, Rubric, Scorecard
...
class Registry:
    ...
    def rubric_of_kind(self, kind: str) -> Rubric | None:
        for rb in self.rubrics.values():
            if rb.kind == kind:
                return rb
        return None

    def frameworks_for_rubric(self, rb: Rubric) -> list[Framework]:
        return [self.frameworks[fid] for fid in rb.frameworks if fid in self.frameworks]

    def crm_mapping(self, crm: str) -> dict:
        """Load a CRM mapping config by name (e.g. 'salesforce')."""
    ...
    def validate(self) -> list[str]:
        ...
        # Rubrics (deal/account) cite real frameworks and elements.
        for rb in self.rubrics.values():
            for fid in rb.frameworks:
                if fid not in self.frameworks:
                    problems.append(f"rubric '{rb.id}' references missing framework '{fid}'")
            for dim in rb.dimensions:
                for ref in dim.framework_refs:
                    if "." not in ref:
                        problems.append(f"rubric '{rb.id}'.{dim.id}: framework_ref '{ref}' is not 'framework.element'")
                        continue
                    fwid, elid = ref.split(".", 1)
                    fw = self.frameworks.get(fwid)
                    if not fw:
                        problems.append(f"rubric '{rb.id}'.{dim.id}: framework_ref '{ref}' -> unknown framework '{fwid}'")
                    elif elid not in {e.id for e in fw.elements}:
                        problems.append(f"rubric '{rb.id}'.{dim.id}: framework_ref '{ref}' -> '{fwid}' has no element '{elid}'")
