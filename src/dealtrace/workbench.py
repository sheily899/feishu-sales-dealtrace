...
function evidenceLinks(evidence){return(evidence||[]).map(x=>{let ids=(state.evidenceMap||{})[evidenceKey(x)]||[],source=(state.messages||[]).find(m=>m.text===x.text);if(!ids.length&&source)ids=[source.messageId];let speaker=evidenceSpeaker(x.speaker)||(source?source.senderName:"");let label='原文依据：'+e(speaker||"未知")+'：'+e(x.text);if(!ids.length)return'<p class="muted">'+label+'</p>';return'<button type="button" class="evidence-link" data-message-id="'+e(ids[0])+'">'+label+'</button>'}).join("")}
...
function renderCustomerState(d){let box=q("#customer-state"),s=d.customerState,c=d.stateChange,h=d.stateHistory||[],lastMessage=(d.messages||[]).slice(-1)[0],fallbackTime=lastMessage&&lastMessage.sentAt;if(!s){box.innerHTML="";return}let stage=s.stage&&s.stage!=="unknown"?s.stage:(d.callTypeLabel||"未识别"),updatedAt=s.updatedAt||fallbackTime||"-";let out='<section class="state-panel"><h3>当前客户状态 · 第 '+e(s.version)+' 次状态更新</h3><p class="muted">更新时间：'+e(updatedAt)+'</p><p><strong>当前阶段：</strong>'+e(stage)+'</p>'+stateItems("已识别需求",s.needs)+stateItems("未解决顾虑",s.unresolvedConcerns)+stateItems("销售承诺",s.commitments)+stateItems("待办事项",s.todos)+stateItems("相关角色",s.stakeholders)+stateItems("商机风险",s.risks)+stateItems("已约定下一步",s.scheduledNextSteps);if(c){out+='<h3>本次变化</h3>'+(c.currentFocus?'<p><strong>当前重点：</strong>'+e(c.currentFocus)+'</p>':"")+changeItems("新增",c.added)+changeItems("已解决",c.resolved)+(c.statusTransitions||[]).map(x=>'<div class="item"><strong>状态迁移：'+e(changeLabels[x.category]||x.category)+'｜'+e(x.title)+'</strong><p>'+e(x.fromStatus)+' → '+e(x.toStatus)+'</p>'+evidenceLinks(x.evidence)+'</div>').join("")}out+='<h3>状态更新记录</h3><ol class="timeline">'+h.map(x=>'<li><strong>第 '+e(x.version)+' 次更新</strong> · '+e(x.updatedAt||fallbackTime||"-")+(x.change&&x.change.currentFocus?'：'+e(x.change.currentFocus):"")+'</li>').join("")+'</ol></section>';box.innerHTML=out}
