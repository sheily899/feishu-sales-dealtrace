...
async function loadChats(){let r=await fetch("/api/chats");if(!r.ok)return;let data=await r.json(),chats=data.chats||[];q("#go").style.display=data.offline_demo?"none":"inline-block";if(!currentChatId&&chats.length)currentChatId=chats[0].chatId;renderChatList(chats)}
