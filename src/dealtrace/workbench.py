    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if path == "/api/chats":
                ...
                self._json({"chats": chats, "offline_demo": not bool(feishu_config)})
...
<main><header><div><p class="eyebrow">SALES INTELLIGENCE</p><h1 id="name">加载中…</h1><p id="meta" class="muted"></p></div><div><button id="offline">离线演示</button> <button id="go">生成分析</button></div></header><section class="workspace"><aside><h2>客户群</h2><div id="chat-list" class="chat-list" aria-label="客户群列表"></div><h2>群聊记录</h2><div id="messages"></div></aside><article><p class="eyebrow">AI ANALYSIS</p><h2>销售沟通分析</h2><p id="status" class="muted">待分析</p><div id="customer-state"></div><div id="report" class="muted"></div></article></section></main>
<script>
let state,currentChatId;
const q=s=>document.querySelector(s),e=v=>String(v||"").replace(/[&<>]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]));
const chatQuery=()=>currentChatId?"?chatId="+encodeURIComponent(currentChatId):"";
const displayTime=value=>{if(!value)return"-";let date=new Date(value);return Number.isNaN(date.getTime())?value:date.toLocaleString("zh-CN",{timeZone:"Asia/Shanghai",year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false}).replace(/\//g,"-")};
const evidenceKey=x=>(x.speaker||"")+"\\n"+(x.text||"");
const normalizeEvidenceText=value=>String(value||"").replace(/^(客户|销售|未知)\s*[：:]\s*/,"").replace(/\s+/g,"").trim();
function evidenceSpeaker(speaker){let value=String(speaker||"");if(!value||value.toLowerCase()==="unknown")return"";if(value.includes("客户")||value==="prospect")return"客户";if(value.includes("销售")||value==="rep")return"销售";return value.replace(/\\s*\\([^)]*\\)/g,"")}
function evidenceLinks(evidence){return(evidence||[]).map(x=>{let text=String(x.text||"").replace(/^(客户|销售|未知)\s*[：:]\s*/,'');let ids=(state.evidenceMap||{})[evidenceKey(x)]||[],source=(state.messages||[]).find(m=>m.text===x.text||m.text===text);if(!ids.length&&source)ids=[source.messageId];let speaker=evidenceSpeaker(x.speaker)||(source?source.senderName:"");let label='原文依据：'+e(speaker||"未知")+'：'+e(text);if(!ids.length)return'<p class="muted">'+label+'</p>';return'<button type="button" class="evidence-link" data-message-id="'+e(ids[0])+'">'+label+'</button>'}).join("")}
function evidenceLinks(evidence){return(evidence||[]).map(x=>{let text=String(x.text||"").replace(/^(客户|销售|未知)\s*[：:]\s*/,"");let normalized=normalizeEvidenceText(text),source=(state.messages||[]).find(m=>{let candidate=normalizeEvidenceText(m.text);return candidate===normalized||candidate.includes(normalized)||normalized.includes(candidate)}),ids=(state.evidenceMap||{})[evidenceKey(x)]||[];if(!ids.length&&source)ids=[source.messageId];let speaker=evidenceSpeaker(x.speaker)||(source?source.senderName:"");let label='原文依据：'+e(speaker||"未知")+'：'+e(text);if(!ids.length)return'<p class="muted">'+label+'</p>';return'<button type="button" class="evidence-link" data-message-id="'+e(ids[0])+'">'+label+'</button>'}).join("")}
...
function renderCustomerState(d){let box=q("#customer-state"),s=d.customerState,c=d.stateChange,h=d.stateHistory||[],lastMessage=(d.messages||[]).slice(-1)[0],fallbackTime=lastMessage&&lastMessage.sentAt;if(!s){box.innerHTML="";return}let stage=s.stage&&s.stage!=="unknown"?s.stage:(d.callTypeLabel||"未识别"),updatedAt=s.updatedAt||fallbackTime||"-";let out='<section class="state-panel"><h3>当前客户状态 · 第 '+e(s.version)+' 次状态更新</h3><p class="muted">更新时间：'+e(displayTime(updatedAt))+'</p><p><strong>当前阶段：</strong>'+e(stage)+'</p>'+stateItems("已识别需求",s.needs)+stateItems("未解决顾虑",s.unresolvedConcerns)+stateItems("销售承诺",s.commitments)+stateItems("待办事项",s.todos)+stateItems("相关角色",s.stakeholders)+stateItems("商机风险",s.risks)+stateItems("已约定下一步",s.scheduledNextSteps);if(c&&s.version>1){out+='<h3>本次变化</h3>'+(c.currentFocus?'<p><strong>当前重点：</strong>'+e(c.currentFocus)+'</p>':"")+changeItems("新增",c.added)+changeItems("已解决",c.resolved)+(c.statusTransitions||[]).map(x=>'<div class="item"><strong>状态迁移：'+e(changeLabels[x.category]||x.category)+'｜'+e(x.title)+'</strong><p>'+e(x.fromStatus)+' → '+e(x.toStatus)+'</p>'+evidenceLinks(x.evidence)+'</div>').join("")}if(h.length>1){out+='<h3>状态更新记录</h3><ol class="timeline">'+h.map(x=>'<li><strong>第 '+e(x.version)+' 次更新</strong> · '+e(displayTime(x.updatedAt||fallbackTime||"-"))+'</li>').join("")+'</ol>'}box.innerHTML=out+'</section>'}
function renderChatList(chats){q("#chat-list").innerHTML=(chats||[]).map(chat=>'<button type="button" class="chat-button" data-chat-id="'+e(chat.chatId)+'" aria-current="'+(chat.chatId===currentChatId)+'"><strong>'+e(chat.displayName)+'</strong><small>'+e(chat.stage||"待分析")+' · 待办 '+e(chat.todoCount)+' 项</small></button>').join("")}
function emptyChat(){state=null;q("#name").textContent="请选择一个客户群";q("#meta").textContent="从左侧选择客户群后查看沟通记录与分析结果";q("#messages").innerHTML="";q("#customer-state").innerHTML="";q("#report").className="muted";q("#report").textContent="请选择一个客户群。";q("#status").textContent="待选择";q("#go").disabled=true}
...
function show(d){state=d;currentChatId=d.chatId||currentChatId;q("#go").disabled=false;q("#name").textContent=d.customerName;q("#meta").textContent="来源："+d.sourceLabel+" · 已同步 "+d.messages.length+" 条有效消息";q("#messages").innerHTML=d.messages.map(m=>'<div class="message '+m.role+'" data-message-id="'+e(m.messageId)+'"><span class="who">'+e(m.senderName)+'</span><span class="time">'+e(displayTime(m.sentAt).slice(11,16))+'</span><p>'+e(m.text)+'</p></div>').join("");renderCustomerState(d);if(d.noNewMessages)q("#status").textContent="暂无新消息，已保留当前客户状态";if(!d.analysis)return;let a=d.analysis,g=a.group_chat,out='<h3>沟通总结</h3><p>'+e(a.summary)+'</p><h3>沟通阶段：'+e(d.callTypeLabel||a.classification.call_type)+'</h3>';if(g){out+=items("销售回应覆盖",g.response_coverage)+items("销售建议",g.next_steps)}else{out+=items("销售表现",a.coaching.strengths)+items("改进建议",a.coaching.improvements)}q("#report").innerHTML=out}
...
async function loadChats(){let r=await fetch("/api/chats");if(!r.ok)return;let data=await r.json(),chats=data.chats||[];q("#go").style.display=data.offline_demo?"none":"inline-block";q("#offline").style.display=data.offline_demo?"inline-block":"none";if(!currentChatId&&chats.length)currentChatId=chats[0].chatId;renderChatList(chats)}
