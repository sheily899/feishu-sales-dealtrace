...
<script>let state;const q=s=>document.querySelector(s),e=v=>String(v||"").replace(/[&<>]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]));function show(d){state=d;q("#name").textContent=d.customerName;q("#meta").textContent="来源："+d.sourceLabel+" · 已同步 "+d.messages.length+" 条有效消息";q("#messages").innerHTML=d.messages.map(m=>'<div class="message '+m.role+'" id="m-'+m.messageId+'"><span class="who">'+e(m.senderName)+'</span><span class="time">'+m.sentAt.slice(11,16)+'</span><p>'+e(m.text)+'</p></div>').join("");if(!d.analysis)return;let a=d.analysis,out='<p>'+e(a.summary)+'</p><h3>沟通阶段：'+e(a.classification.call_type)+'</h3>';[["客户目标",a.outcomes],["销售表现",a.coaching.strengths],["改进建议",a.coaching.improvements]].forEach(x=>{if(x[1]&&x[1].length)out+='<h3>'+x[0]+'</h3>'+x[1].map(i=>'<div class="item"><button data-evidence="'+e(JSON.stringify(i.evidence||[]))+'">'+e(i.title||i.statement)+'</button><p>'+e(i.detail||i.status)+'</p></div>').join("")});q("#report").innerHTML=out}document.addEventListener("click",x=>{if(!x.target.dataset.evidence)return;let ev=JSON.parse(x.target.dataset.evidence)[0];if(!ev)return;let m=state.messages.find(i=>(ev.text||"").includes(i.text)||i.text.includes(ev.text||""));if(m){let n=q("#m-"+m.messageId);n.classList.add("highlight");n.scrollIntoView({behavior:"smooth",block:"center"})}});q("#go").onclick=async()=>{let b=q("#go");b.disabled=true;b.textContent="分析中…";q("#status").textContent="DeepSeek 分析中";try{let r=await fetch("/api/analyze",{method:"POST"}),d=await r.json();if(!r.ok)throw Error(d.error);show(d);q("#status").textContent="分析完成";b.textContent="重新分析"}catch(x){q("#report").className="error";q("#report").textContent="分析失败："+x.message;q("#status").textContent="分析失败"}finally{b.disabled=false}};fetch("/api/workbench").then(r=>r.json()).then(show)</script>"""
