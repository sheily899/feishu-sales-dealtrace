...
async function load(){try{let r=await fetch("/api/workbench"+chatQuery());if(!r.ok)throw Error("群聊同步失败");show(await r.json())}catch(x){q("#status").textContent="同步失败"}}
